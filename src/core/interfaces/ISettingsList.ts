...
export interface ISettingsList {
   ...
   * (`percentTime`, a percentage of the available musical time the
   * generation process has to "fill"). If two calls use the same
   * `percentTime`, the later call overwrites the former.
   *
   * @param parameter The parameter the value refers to.
   * @param percentTime A point in time (`0`-`100` inclusive) referring
   * to the available musical duration the generation process has to
   * "fill". E.g. `50` would point to the start of the third measure in a
   * four-measure-long fragment.
  ...
  /**
   * Retrieves the value recorded for `parameter` at `percentTime`. If no
   ...
   *
   * @param parameter The parameter the value refers to.
   * @param percentTime A point in time (`0`-`100` inclusive).
